...
<template>
  <li class="row px-3" :class="{ unread: !isRead }">
    ...
    <p class="msg text-end">
      <template v-if="hasLink">
        <span v-if="notification.message_prefix">{{
          notification.message_prefix
        }}</span>
        <NuxtLink
        ...
        <span v-if="notification.message_suffix">{{
          notification.message_suffix
        }}</span>
      </template>
      <template v-else>
